...
export const Heading: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <h2 className={cn('sm:text-5xl leading-[1] text-[clamp(1.7rem,10vw,3rem)] font-bold', className)}>
    {children}
  </h2>
)
...
export const Paragraph: React.FC<{ children: React.ReactNode; className?: string }> = ({
  ...
  <p className={cn('sm:text-2xl text-[clamp(0.1rem,20vw,1.25rem)] font-bold opacity-80', className)}>
    {children}
  </p>
)
...
export const PricingWrapper: React.FC<{
  ...
  contactLabel?: string
  className?: string
}> = ({ children, contactHref, contactLabel = 'View Details', className, type = 'waves' }) => (
  <article
    className={cn(
      'min-h-[300px] h-[560px] max-h-[560px] max-w-sm w-full bg-purple-500 relative overflow-hidden rounded-2xl text-white border-2 border-[#1a1a1a]',
      className
    )}
  >
    <div
      className="w-full h-full absolute top-0 left-0 z-[2] p-6 flex flex-col items-start justify-start sm:gap-8 gap-6"
    >
      {children}
      <div className="w-full h-full flex items-end justify-end text-base">
        <Link href={contactHref} className="w-full h-fit">
          <div className="h-12 w-full bg-white rounded-xl text-neutral-900 font-bold text-sm hover:bg-neutral-100 transition-colors border-2 border-neutral-900/10 flex items-center justify-center">
            {contactLabel}
          </div>
        </Link>
      </div>
